...
contract('Usamos un Contador:', accounts => {
    ...
    it("incrementa en cuatro el contador y lo decrementa a uno", async () => {
        ...
        const incr = c2.sub(c1);
        assert.equal(incr.toNumber(), 1, "El decremento del valor no es 1.");
    });

    it("incrementa en cuatro el contador y lo reinicia a cero", async () => {
        ...
        let c = await contador.valor.call();

        assert.equal(c.toNumber(), 0, "El contador no se ha reiniciado a cero.");
    });
